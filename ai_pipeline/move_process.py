...
def get_command_and_movement(text):
    ...
    prompt = """
            Given the following string of text, produce a JSON object. The JSON array should be formatted as ["actions": [{"command": command, "radians"?: radians_number, "metres"?: metres: metres_number}, ...]] where metres is chosen if we select a MOVE command 
    ...
    However, if the user doesn't specify a number (e.g. they say "turn right a little bit"), use your judgement to determine a suitable amount for the output (e.g. a little is 0.5 radians, left turn is 1.7 radians, a half circle is 3.2 radians, full circle is 6.4 radians)
            """
...
def main():
    input_text = "Hey Spot, look behind you and then turn back around, then look up."
